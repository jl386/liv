...
function getSignalCliBin(): string {
  // Check node_modules first, then fall back to system PATH
  ...
  // Check if signal-cli is in system PATH
  try {
    execSync('which signal-cli', { encoding: 'utf-8', timeout: 5000 });
    return 'signal-cli';
  } catch {
    // not found in PATH either
  }
  return localBin; // return default path so the error message is clear
}

function getJavaEnv(): Record<string, string> {
  const env = { ...process.env } as Record<string, string>;

  // Check SDKMAN Java first (Linux, e.g. Raspberry Pi)
  const sdkmanJavaDir = path.join(os.homedir(), '.sdkman', 'candidates', 'java', 'current');
  if (fs.existsSync(path.join(sdkmanJavaDir, 'bin', 'java'))) {
    env.JAVA_HOME = sdkmanJavaDir;
    env.PATH = `${sdkmanJavaDir}/bin:${env.PATH || ''}`;
    return env;
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  const env = readEnvFile(['SIGNAL_BOT_PHONE', 'SIGNAL_PHONE_NUMBER']);
  const phoneNumber =
    process.env.SIGNAL_BOT_PHONE || env.SIGNAL_BOT_PHONE ||
  ...
  const signalCliBin = getSignalCliBin();
  // If it's an absolute path, check it exists; if bare command (from PATH), trust it
  if (signalCliBin.includes('/') && !fs.existsSync(signalCliBin)) {
    console.log('SIGNAL_AUTH_OK=false');
    console.log('STATUS=error');
    console.log('ERROR=signal-cli binary not found. Install signal-cli or run: npm install signal-sdk');
    console.log('=== END ===');
    return;
  }
